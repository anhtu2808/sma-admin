...
const Logo = ({ className = '', iconColor = '', collapsed = false }) => {
  return (
    <Link to="/" className={`flex items-center gap-2 group ${collapsed ? 'justify-center' : ''} ${className}`}>
      ...
      />
      {!collapsed && (
        <span className={`text-xl font-bold whitespace-nowrap ${iconColor === 'white' ? 'text-white' : 'dark:text-white text-gray-900'} tracking-tight group-hover:text-primary transition-colors`}>
          SmartRecruit
        </span>
      )}
    </Link>
